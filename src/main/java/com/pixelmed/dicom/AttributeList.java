...
public class AttributeList extends TreeMap {
	...
	/***/
	private static final String identString = "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/dicom/AttributeList.java,v 1.1.1.1 2011-05-21 10:08:46 nicola Exp $";
	...
	public void removePrivateAttributes() {
		...
		while (i.hasNext()) {
			...
			if (a.getTag().isPrivate()) {
				...
				Iterator i2 = ((SequenceAttribute)a).iterator();
				while (i2.hasNext()) {
					SequenceItem item = (SequenceItem)i2.next();
					if (item != null) {
	...
	public static void main(String arg[]) {
		if (arg.length == 3 && arg[2].equals("TEST")) {
			...
			if (arg.length == 1) {
				...
				outputTransferSyntax = TransferSyntaxFromName.getUID(arg[2]);
				outputFileName = arg[3];
				outputMeta = ! arg[4].toUpperCase().contains("NO");
			}
			try {
				AttributeList list = new AttributeList();
