...
public abstract class ContentItem implements TreeNode {
	...
	private static final String identString = "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/dicom/ContentItem.java,v 1.1.1.1 2011-05-21 10:08:46 nicola Exp $";
...
//System.err.println("ContentItem.addChild(): child = "+child);
	...
	public String getSingleStringValueOrNullOfNamedChild(String codingSchemeDesignator,String codeValue) {
		String value = null;
		{
			...
			if (child != null) {
				if (child instanceof ContentItemFactory.StringContentItem) {
					value = ((ContentItemFactory.StringContentItem)child).getConceptValue();
				}
				else if (child instanceof ContentItemFactory.CodeContentItem) {
					value = ((ContentItemFactory.CodeContentItem)child).getConceptValue();		// will return CodeMeaning
				}
				else if (child instanceof ContentItemFactory.NumericContentItem) {
					value = ((ContentItemFactory.NumericContentItem)child).getNumericValue();	// NOT getConceptValue(), which includes the units
				}
			}
		}
		return value;
