 ...
 *
 * <p>
 * UID attributes are handled specially, in that they may be kept, removed or remapped. Remapping
 ...
 *
 * <p>
 * Note that this map could grow quite large and consumes resources in memory, and hence in a server
...
abstract public class ClinicalTrialsAttributes
{
  ...
  /**  */
  private static final String identString = "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/dicom/ClinicalTrialsAttributes.java,v 1.1.1.1 2011-05-21 10:08:46 nicola Exp $";
  ...
  /**
   * <p>
   * Add the attributes of the Contributing Equipment Sequence to a list of attributes.</p>
   *
   * <p>
   * Attributes are added if supplied string value are added if not null. May be zero length.</p>
   *
   * <p>
   * Retains any existing items in Contributing Equipment Sequence.</p>
   *
   * <p>
   * Uses <code>("109104","DCM","De-identifying Equipment")</code> for the Purpose of Reference.</p>
   *
   * <p>
   * Uses <code>"Deidentified"</code> for the Contribution Description.</p>
   *
   * <p>
   * Uses the current date and time for the Contribution DateTime.</p>
   *
  ...
  /**
   * <p>
   * Remove the attributes of the Clinical Trials Patient, Study and Series Modules, from a list of attributes.</p>
   *
  ...
  /**
   * <p>
   * Add the attributes of the Clinical Trials Patient, Study and Series Modules, to a list of attributes.</p>
   *
  ...
  /**
   * <p>
   * Is a private tag safe.</p>
   *
   * <p>
   * Safe private attributes are all those that are known not to contain individually identifiable information.</p>
   *
   * <p>
   * Private creators are always considered safe.</p>
   *
...
//System.err.println("ClinicalTrialsAttributes.isSafePrivateAttribute(): safe="+safe);
   ...
   *
   * <p>
   * Private creators are always considered safe, though there is no point in calling this method for private creator tags ... use AttributeTag.isPrivateCreator() instead.</p>
   *
...
//System.err.println("ClinicalTrialsAttributes.isSafePrivateAttribute(): checking "+tag+" creator "+creator);
    ...
    else
    {
      ...
      if(group == 0x7053)
      {
        ...
        {
          if(elementInBlock == 0x0000)
          {			// DS	SUV Factor – Multiplying stored pixel values by Rescale Slope then this factor results in SUVbw in g/l
            safe = true;
          }
          else if(elementInBlock == 0x0009)
          {	// DS	Activity Concentration Factor – Multiplying stored pixel values by Rescale Slope then this factor results in MBq/ml.
            safe = true;
          }
        }
      }
      else if(group == 0x00E1)
      {
        if(creator.equals("ELSCINT1"))
        {
          if(elementInBlock == 0x0021)
          {			// DS	DLP
            safe = true;
          }
        }
      }
      else if(group == 0x01E1)
      {
        if(creator.equals("ELSCINT1"))
        {
          if(elementInBlock == 0x0026)
          {			// CS	Phantom Type
            safe = true;
          }
          else if(elementInBlock == 0x0050)
          {	// DS	Acquisition Duration
            safe = true;
          }
        }
      }
      else if(group == 0x01F1)
      {
        if(creator.equals("ELSCINT1"))
        {
          if(elementInBlock == 0x0001)
          {			// CS	Acquisition Type
            safe = true;
          }
          else if(elementInBlock == 0x0007)
          {	// DS	Table Velocity
            safe = true;
          }
          else if(elementInBlock == 0x0026)
          {	// DS	Pitch
            safe = true;
          }
          else if(elementInBlock == 0x0027)
          {	// DS	Rotation Time
            safe = true;
          }
        }
      ...
      {
        if(creator.equals("GEMS_ACQU_01"))
        {
          if(elementInBlock == 0x0023)
          {			// DS	Table Speed [mm/rotation]
            safe = true;
          }
          else if(elementInBlock == 0x0024)
          {	// DS	Mid Scan Time [sec]
            safe = true;
          }
          else if(elementInBlock == 0x0027)
          {	// DS	Rotation Speed (Gantry Period)
            safe = true;
          }
        }
      }
      else if(group == 0x0043)
      {
        if(creator.equals("GEMS_PARM_01"))
        {
          if(elementInBlock == 0x0027)
          {			// SH	Scan Pitch Ratio in the form "n.nnn:1"
            safe = true;
          }
        }
      }
      else if(group == 0x0045)
      {
        if(creator.equals("GEMS_HELIOS_01"))
        {
          if(elementInBlock == 0x0001)
          {			// SS	Number of Macro Rows in Detector
            safe = true;
          }
          else if(elementInBlock == 0x0002)
          {	// FL	Macro width at ISO Center
            safe = true;
          }
        }
      }
      else if(group == 0x0903)
      {
        if(creator.equals("GEIIS PACS"))
        {
          if(elementInBlock == 0x0010)
          {			// US	Reject Image Flag
            safe = true;
          }
          else if(elementInBlock == 0x0011)
          {	// US	Significant Flag
            safe = true;
          }
          else if(elementInBlock == 0x0012)
          {	// US	Confidential Flag
            safe = true;
          }
        }
      }
  ...
  /**
   * <p>
   * Flush (remove all entries in) the map of original UIDs to replacement UIDs.</p>
   */
  ...
  /**
   * <p>
   * Remap UID attributes in a list of attributes, recursively iterating through nested sequences.</p>
   *
  ...
  /**
   * <p>
   * Remove UID attributes in a list of attributes, recursively iterating through nested sequences.</p>
   *
   * @param	list	the list of attributes to be cleaned up
   * @throws	DicomException
   */
  ...
  /**
   * <p>
   * Remove or remap UID attributes in a list of attributes, recursively iterating through nested sequences.</p>
   *
   * @param	list	the list of attributes to be cleaned up
...
//if (tag.equals(TagFromName.SOPInstanceUID)) { System.err.println("ClinicalTrialsAttributes.removeOrRemapUIDAttributes(): replacing SOP Instance UID "+originalUIDValue+" with "+replacementUIDValue); }
  ...
  /**
   * <p>
   * Deidentify a list of attributes.</p>
   *
   * <p>
   * De-identifies attributes within nested sequences, other than Context Sequence.</p>
   *
   * <p>
   * Handles UIDs as requested, including within nested sequences, including Context Sequence.</p>
   *
   * <p>
   * Also adds record that de-identification has been performed.</p>
   *
   * @deprecated	retained for compatibility with previous releases; does NOT keep patient characteristics (such as might be needed for PET SUV calculations); use
   * {@link #removeOrNullIdentifyingAttributes(AttributeList,boolean,boolean,boolean) removeOrNullIdentifyingAttributes()} instead
   *
   * @param	list	the list of attributes to be cleaned up
   ...
   */
  @Deprecated
  public static void removeOrNullIdentifyingAttributes(AttributeList list, boolean keepUIDs, boolean keepDescriptors)
     throws DicomException
  {
   ...
   * @throws	DicomException
   */
  public static void removeOrNullIdentifyingAttributes(AttributeList list, boolean keepUIDs, boolean keepDescriptors, boolean keepPatientCharacteristics)
     throws DicomException
  {
   ...
   * @throws	DicomException
   */
  public static void removeOrNullIdentifyingAttributes(AttributeList list, int handleUIDs, boolean keepDescriptors, boolean keepPatientCharacteristics)
     throws DicomException
  {
  ...
  /**
   * <p>
   * Deidentify a list of attributes, recursively iterating through nested sequences.</p>
   *
   * <p>
   * Does not process UIDs, but does remove sequences that would be invalidated by removing UIDs, e.g., Source Image Sequence and Referenced Image Sequence.</p>
   *
   * @param	list	the list of attributes to be cleaned up
   * @param	handleUIDs	keep, remove or remap the UIDs
   * @param	keepDescriptors	if true, keep the text description and comment attributes
   ...
   * @throws	DicomException
   */
  protected static void removeOrNullIdentifyingAttributesRecursively(AttributeList list, int handleUIDs, boolean keepDescriptors, boolean keepSeriesDescriptors, boolean keepPatientCharacteristics, boolean keepDeviceIdentity, boolean keepInstitutionIdentity)
     throws DicomException
  {
    ...
    while(i.hasNext())
    {
      ...
      {
        SequenceAttribute a = (SequenceAttribute) o;
        if(!a.getTag().equals(TagFromName.ContentSequence))
        {	// i.e. do NOT descend into SR content tree
          Iterator items = a.iterator();
          if(items != null)
          {
            while(items.hasNext())
...
//System.err.println("Recursed into item of "+a);
   ...
   *
   * <p>
   * Read a DICOM object from the file specified on the command line, and remove identifying attributes, and add sample clinical trials attributes.</p>
   *
   * @param	arg
   */
  public static void main(String arg[])
  {
    ...
    try
    {
      ...
      CodedSequenceItem[] operatorPersonIdentificationCodeSequence1 =
      {
        new CodedSequenceItem("634872364", "99MYH", "Smith^John")
      };
      CodedSequenceItem[] operatorPersonIdentificationCodeSequence2 =
      {
        new CodedSequenceItem("346234622", "99MYH", "Doe^Jane")
      };
      ...
      operatorIdentifications[0]
         = new PersonIdentification(operatorPersonIdentificationCodeSequence1, "John address", phoneNumbers1, null, "My hospital address", new CodedSequenceItem("47327864", "99MYH", "My Hospital"));
