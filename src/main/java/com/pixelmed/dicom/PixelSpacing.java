...
public class PixelSpacing {
	...
	private static final String identString = "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/dicom/PixelSpacing.java,v 1.1.1.1 2011-05-21 10:08:46 nicola Exp $";
	...
	public PixelSpacing(AttributeList list,GeometryOfVolume volumeGeometry,boolean preferCalibratedValue,boolean useMagnificationFactorIfPresent) {
		...
		if (!found) {
			...
			if (!found || (preferCalibratedValue && list.get(TagFromName.PixelSpacing) != null)) {
				String calibrationString = Attribute.getSingleStringValueOrDefault(list,TagFromName.PixelSpacingCalibrationType,"calibrated").toLowerCase();
				calibrationString = Attribute.getSingleStringValueOrDefault(list,TagFromName.PixelSpacingCalibrationDescription,calibrationString).toLowerCase();
