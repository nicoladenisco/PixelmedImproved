...
public class ContentItemFactory {
	...
	/***/
	private static final String identString = "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/dicom/ContentItemFactory.java,v 1.1.1.1 2011-05-21 10:08:46 nicola Exp $";
		...
		public ContainerContentItem(ContentItem parent,String relationshipType,CodedSequenceItem conceptName,boolean continuityOfContentIsSeparate,
			...
			if (this.templateMappingResource != null || templateIdentifier != null) {
				...
				if (templateMappingResource != null) {
					Attribute a = new CodeStringAttribute(TagFromName.MappingResource); a.addValue(templateMappingResource.toUpperCase()); contentTemplateSequenceItemAttributeList.put(a);
				}
				if (templateIdentifier != null) {
					Attribute a = new CodeStringAttribute(TagFromName.TemplateIdentifier); a.addValue(templateIdentifier.toUpperCase()); contentTemplateSequenceItemAttributeList.put(a);
				}
		...
		public String toString() {
			...
			if (graphicData != null) {
				for (int j=0; j<graphicData.length; ++j) {
					if (j > 0) str.append(",");
					str.append(FloatFormatter.toStringOfFixedMaximumLength(graphicData[j],16/* maximum size of DS */,false/* non-numbers already handled */));
				}
			}
			str.append(")");
			return str.toString();
...
//System.err.println("NumericContentItem(): matches NaN");
				...
				doCommonConstructorStuff(FloatFormatter.toStringOfFixedMaximumLength(numericValue,16/* maximum size of DS */,false/* non-numbers already handled */),units,null/*no qualifier*/);
			}
		}
