...
/**
 * <p>
 * The {@link com.pixelmed.query.QueryInformationModel QueryInformationModel} class is an abstract class that contains the core
 * functionality for performing DICOM query and retrieval over the network.</p>
 *
 * <p>
 * It hides the underlying DICOM network implementation.</p>
 *
 * <p>
 ...
 *
 * <p>
 * The majority of methods are protected and are for the benefit of those implementing their
...
abstract public class QueryInformationModel
{
   ...
   */
  private AttributeList makeIdentifierFromAttributesAtThisQueryLevel(AttributeList oldList, AttributeList parentUniqueKeys, InformationEntity queryLevel)
     throws DicomException
  {
    if(debugLevel > 1)
      System.err.println("makeIdentifierFromAttributesAtThisQueryLevel: queryLevel=" + queryLevel);
...
//System.err.println("makeIdentifierFromAttributesAtThisQueryLevel: adding "+a);
    ...
    }
    {
      ...
      a.addValue(getQueryLevelName(queryLevel));
      newList.put(t, a);
    }
    if(parentUniqueKeys != null)
      newList.putAll(parentUniqueKeys);
    ...
    if(debugLevel > 1)
      System.err.println("makeIdentifierFromAttributesAtThisQueryLevel: identifier=" + newList);
    return newList;
  }
  ...
  /**
   * <p>
   * Perform a single level query and return the response to the specified handler.</p>
   *
   ...
   */
  void performQuery(AttributeList filter, AttributeList parentUniqueKeys, InformationEntity queryLevel, IdentifierHandler responseIdentifierHandler)
     throws IOException, DicomException, DicomNetworkException
  {
    if(debugLevel > 0)
      System.err.println("QueryInformationModel.performQuery(): queryLevel=" + queryLevel);
    if(debugLevel > 0)
      System.err.println("QueryInformationModel.performQuery(): parentUniqueKeys=" + parentUniqueKeys);
    AttributeList requestIdentifier = makeIdentifierFromAttributesAtThisQueryLevel(filter, parentUniqueKeys, queryLevel);
    if(debugLevel > 1)
      System.err.println("QueryInformationModel.performQuery(): requestIdentifier=" + requestIdentifier);
  ...
  /**
   * <p>
   * Perform a hierarchical query and return the response as a tree.</p>
   *
   * <p>
   * Performs a query recursively from the requested level of the information model
   ...
   *
   * <p>
   * It starts out at the highest level of the model, and for each response returned
   ...
   *
   * <p>
   * The actual queries at lower levels may be deferred and not performed until
   * the tree is actually expanded whilst browsing, to avoid delays in making the top level nodes
   * available.</p>
   *
   * <p>
   * If the filter contains the attributes to count the number of subsidiary entities (e.g.,
  ...
  /**
   * <p>
   * Retrieve DICOM object(s).</p>
   *
   * <p>
   * Assumes that the objects are available at the<code>calledAETitle</code>
   * specified in the constructor in this class instance.</p>
   *
   * <p>
   * Assumes that we have a storage SCP listening as the <code>callingAETitle</code>
   * specified in the constructor in this class instance.</p>
   *
   ...
   */
  @Deprecated
  public void performHierarchicalMove(AttributeList identifier, String retrieveAE)
     throws IOException, DicomException, DicomNetworkException
  {
  ...
  /**
   * <p>
   * Retrieve DICOM object(s) from the specified location.</p>
   *
   * <p>
   * Assumes that we have a storage SCP listening as the <code>callingAETitle</code>
   * specified in the constructor in this class instance.</p>
   *
   * <p>
   * Note that the <code>retrieveAE</code> argument may differ from the
   ...
   * @exception	DicomNetworkException	thrown if there is a problem with the DICOM network protocol
   */
  public void performHierarchicalMoveFrom(AttributeList identifier, String retrieveAE)
     throws IOException, DicomException, DicomNetworkException
  {
  ...
  /**
   * <p>
   * Retrieve DICOM object(s) to the specified location.</p>
   *
   * <p>
   * Assumes that the objects are available at the <code>calledAETitle</code>
   * specified in the constructor in this class instance.</p>
   *
   * <p>
   * Further assumes that <code>calledAETitle</code> knows how to resolve the <code>moveDestination</code>
   ...
   * @exception	DicomNetworkException	thrown if there is a problem with the DICOM network protocol
   */
  public void performHierarchicalMoveTo(AttributeList identifier, String moveDestination)
     throws IOException, DicomException, DicomNetworkException
  {
  ...
  /**
   * <p>
   * Retrieve DICOM object(s) from the specified location to the specified location.</p>
   *
   * @param	identifier	the move request identifier as a list of DICOM attributes
   * @param	retrieveAE	the AE title of where to move the object(s) from
   * @param	moveDestination	the AE title of where to move the object(s) to
   ...
   * @exception	DicomNetworkException	thrown if there is a problem with the DICOM network protocol
   */
  public void performHierarchicalMoveFromTo(AttributeList identifier, String retrieveAE, String moveDestination)
     throws IOException, DicomException, DicomNetworkException
  {
  ...
  /**
   * <p>
   * Construct a query information model.</p>
   *
   * <p>
   * Does not actually open an association or perform a query or retrieval; for that see:</p>
   * <ul>
   * <li> {@link #performHierarchicalQuery(AttributeList) performHierarchicalQuery()}
   * <li> {@link #performHierarchicalMove(AttributeList) performHierarchicalMove()}
   * <li> {@link #performHierarchicalMoveFrom(AttributeList,String) performHierarchicalMoveFrom()}
