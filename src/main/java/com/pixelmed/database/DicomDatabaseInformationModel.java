...
/**
 * <p>
 * The {@link com.pixelmed.database.DicomDatabaseInformationModel DicomDatabaseInformationModel} class
 ...
 *
 * <p>
 * These methods include support for naming the standard DICOM information entities,
...
public abstract class DicomDatabaseInformationModel extends DatabaseInformationModel
{
  ...
  /**  */
  private static final String identString = "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/database/DicomDatabaseInformationModel.java,v 1.1.1.1 2011-05-21 10:08:46 nicola Exp $";
   ...
   */
  public DicomDatabaseInformationModel(String databaseFileName, InformationEntity rootInformationEntity, DicomDictionary dictionary)
   ...
   * @exception	DicomException
   */
  public DicomDatabaseInformationModel(String databaseFileName, InformationEntity rootInformationEntity, DicomDictionary dictionary, String databaseRootName)
     throws DicomException
  {
   ...
   * @exception	DicomException
   */
  public DicomDatabaseInformationModel(String databaseFileName, String databaseServerName, InformationEntity rootInformationEntity, DicomDictionary dictionary)
     throws DicomException
  {
   ...
   * @exception	DicomException
   */
  public DicomDatabaseInformationModel(String databaseFileName, String databaseServerName, InformationEntity rootInformationEntity, DicomDictionary dictionary, String databaseRootName)
     throws DicomException
  {
...
//System.err.println("DicomDatabaseInformationModel.extendInsertStatementStringWithPersonNameSearchValuesForSelectedInformationEntity(): "+newColumnName+"="+quotedValue);
   ...
   *
   * <p>
   * The default implementation adds values for all attributes that are present in the database tables
   * and present in the instance.</p>
   *
   * <p>
   * At the INSTANCE level, InstanceNumber is always added.</p>
   *
   * <p>
   * At the INSTANCE level, ImageOrientationPatient within PlaneOrientationSequence within SharedFunctionalGroupsSequence is always added, if present.</p>
   *
   * @param	b	the statement being constructed
   * @param	list	the DICOM attributes of a composite object, containing the attributes describing this instance of the entity
     ...
     throws DicomException
  {
    ...
    while(i.hasNext())
    {
      ...
      if(ie == dictionary.getInformationEntityFromTag(tag))
      {
        ...
        {								// do not add same attribute twice to same table
          b.append(",");
          b.append(columnName);
   ...
   *
   * <p>
   * At the INSTANCE level, the file reference type is left empty since unknown.</p>
   *
   * @deprecated use
    ...
    if(time != null && (l = time.length()) > 0)
    {
      if(time.indexOf(':') == -1)
      {
        ...
        if(l >= 2)
          timeFormatString = "HH";
        if(l >= 4)
          timeFormatString = timeFormatString + "mm";
        if(l >= 6)
          timeFormatString = timeFormatString + "ss";
        if(l > 7 && time.indexOf('.') == 6)
        {
          ...
          {
            while(numberOfFractionalSecondDigits++ < 3)
              time = time + "0";
...
//System.err.println("DicomDatabaseInformationModel.getDateFromDicomDateAndTime(dateTime): attribute value dateTime="+dateTime);
    ...
    if(dateTime != null && (l = dateTime.length()) > 0)
    {
      ...
      if(l > 15)
      {
        ...
        if(timezoneStart == -1)
          timezoneStart = dateTime.indexOf('-');
        ...
        if(fractionalSecondStart == 14)
        {
          ...
          {
            while(numberOfFractionalSecondDigits++ < 3)
              fractionalSecondDigits = fractionalSecondDigits + "0";
...
//System.err.println("DicomDatabaseInformationModel.extendInsertStatementStringWithDerivedAttributeValuesForSelectedInformationEntity(): acquisitionDateTime="+acquisitionDateTime);
  ...
  /**
   * <p>
   * Make a quoted string value suitable for using in a SQL statement from a (possibly null or empty) string.</p>
   *
   * <p>
   * No attenpt at escaping specially characters is made ... this is assumed already done.</p>
   *
  ...
  /**
   * <p>
   * Make a quoted string value suitable for using in a SQL statement from a DICOM attribute.</p>
   *
   * <p>
   * Special characters should be escaped (actually they are just replaced with a hyphen).</p>
   *
   * <p>
   * Multiple values are collapsed and separated by the usual DICOM backslash delimiter character (which doesn't bother SQL).</p>
   *
   ...
   *
   * <p>
   * Does NOT escape special characters.</p>
   *
   * <p>
   * Multiple values are collapsed and separated by the usual DICOM backslash delimiter character (which doesn't bother SQL).</p>
   *
   ...
   *
   * <p>
   * Special characters may be escaped (actually they are just replaced with a hyphen).</p>
   *
   * <p>
   * Multiple values are collapsed and separated by the usual DICOM backslash delimiter character (which doesn't bother SQL).</p>
   *
   ...
   * @exception	DicomException
   */
  public static String getQuotedSingleStringValueOrNull(Attribute a, boolean escapeSpecialCharacters)
     throws DicomException
  {
    StringBuffer b = new StringBuffer();
    if(a == null)
    ...
    else
    {
      ...
      {		// only VM of 1 allowed - use 1st if multiple values; also check range (e.g. some Philips Series Numbers violate IS range)
...
//System.err.println("Suppressing too long integer value "+Long.toString(iv[0]));
      ...
      {		// only VM of 1 allowed - use 1st if multiple values
        if(v[0] == null || v[0].trim().length() == 0)
        {
          b.append("NULL");		// need to check string, since a string numeric that is empty will return 0 in Attribute.getLongValues(a), and we prefer NULL to 0
        }
