...
public class StorageSOPClassSCU extends SOPClass
{
  ...
  private static final String identString =
     "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/network/StorageSOPClassSCU.java,v 1.2 2011-06-13 12:34:14 nicola Exp $";
  /***/
     ...
     throws AReleaseException, DicomNetworkException, DicomException, IOException
  {
    byte cStoreRequestCommandMessage[] =
       new CStoreRequestCommandMessage(affectedSOPClass, affectedSOPInstance, 
       moveOriginatorApplicationEntityTitle, moveOriginatorMessageID).getBytes();
...
//if (debugLevel > 0) System.err.println("StorageSOPClassSCU.sendOneSOPInstance(): back from raw binary copy");
    ...
    {
      if(debugLevel > 0)
        System.err.println("StorageSOPClassSCU.sendOneSOPInstance(): different transfer syntaxes; converting " + inputTransferSyntaxUID + " to " + outputTransferSyntaxUID);
      ...
      DicomOutputStream dout =
         new DicomOutputStream(out, null/*meta*/, outputTransferSyntaxUID/*dataset*/);
  ...
  {
    byte cStoreRequestCommandMessage[] =
       new CStoreRequestCommandMessage(affectedSOPClass,
       affectedSOPInstance, moveOriginatorApplicationEntityTitle, moveOriginatorMessageID).getBytes();
    ...
    OutputStream out = association.getAssociationOutputStream(presentationContextID);
    if(debugLevel > 0)
      System.err.println("StorageSOPClassSCU.sendOneSOPInstance(): writing attribute list as " + outputTransferSyntaxUID);
   ...
   */
  public void send(String hostname, int port, String calledAETitle, String callingAETitle,
  ...
  {
    send(hostname, port, calledAETitle, callingAETitle, dicomFiles,
       compressionLevel, multipleInstanceTransferStatusHandler, null, -1, debugLevel);
  }
     ...
     throws DicomNetworkException, IOException
  {
    ...
    try
    {
      LinkedList presentationContexts =
         PresentationContextListFactory.createNewPresentationContextList(dicomFiles, compressionLevel);
      ...
      sendMultipleSOPInstances(association, dicomFiles,
   ...
   * @exception	DicomNetworkException
   */
  public StorageSOPClassSCU(String hostname, int port,
     String calledAETitle, String callingAETitle, String fileName,
  ...
  {
    this(hostname, port, calledAETitle, callingAETitle, fileName,
       affectedSOPClass, affectedSOPInstance, compressionLevel, null, -1, debugLevel);
  }
  ...
  public StorageSOPClassSCU(String hostname, int port,
     String calledAETitle, String callingAETitle, String fileName,
     String affectedSOPClass, String affectedSOPInstance,
     int compressionLevel, String moveOriginatorApplicationEntityTitle, int moveOriginatorMessageID,
     int debugLevel) throws DicomNetworkException, DicomException, IOException
  {
    this.debugLevel = debugLevel;

    if(debugLevel > 0)
      System.err.println("StorageSOPClassSCU: storing " + fileName);
    ...
    try
    {
      ...
      if(din.haveMetaHeader())
      {
        ...
        if(debugLevel > 0)
          System.err.println("Meta header information = " + metaList);
        affectedSOPClass =
           Attribute.getSingleStringValueOrNull(metaList, TagFromName.MediaStorageSOPClassUID);
        affectedSOPInstance =
           Attribute.getSingleStringValueOrNull(metaList, TagFromName.MediaStorageSOPInstanceUID);
        inputTransferSyntax =
           Attribute.getSingleStringValueOrNull(metaList, TagFromName.TransferSyntaxUID);
      }
      else
      {
      ...
      if(debugLevel > 0)
        System.err.println("Using inputTransferSyntax " + inputTransferSyntax);
      ...
      LinkedList presentationContexts = PresentationContextListFactory.createNewPresentationContextList(
         affectedSOPClass, inputTransferSyntax, compressionLevel);
      ...
      if(debugLevel > 0)
        System.err.println("Using context ID " + presentationContextID);
      String outputTransferSyntax =
         association.getTransferSyntaxForPresentationContextID(presentationContextID);
      if(debugLevel > 0)
        System.err.println("Using outputTransferSyntax " + outputTransferSyntax);
   ...
   * @param	debugLevel				zero for no debugging messages, higher values more verbose messages
   */
  public StorageSOPClassSCU(Association association, SetOfDicomFiles dicomFiles,
     MultipleInstanceTransferStatusHandler multipleInstanceTransferStatusHandler,
     int debugLevel)
  {
     ...
     String moveOriginatorApplicationEntityTitle, int moveOriginatorMessageID, int debugLevel)
  {
    ...
    {
      LinkedList presentationContexts =
         PresentationContextListFactory.createNewPresentationContextList(setOfSOPClassUIDs, compressionLevel);

      Association association =
         AssociationFactory.createNewAssociation(hostname, port, calledAETitle, callingAETitle, presentationContexts, null, false, debugLevel);

      sendMultipleSOPInstances(association, lists,
         multipleInstanceTransferStatusHandler,
         moveOriginatorApplicationEntityTitle, moveOriginatorMessageID);
   ...
   * @exception	IOException
   */
  protected void sendMultipleSOPInstances(Association association, SetOfDicomFiles dicomFiles,
     MultipleInstanceTransferStatusHandler multipleInstanceTransferStatusHandler,
     String moveOriginatorApplicationEntityTitle, int moveOriginatorMessageID)
     throws AReleaseException, DicomNetworkException, IOException
  {
    ...
    int nWarning = 0;
    {
      ...
      while(fi.hasNext())
      {
        ...
        if(debugLevel > 0)
          System.err.println("Sending " + fileName);
        boolean success = false;
        ...
        try
        {
          ...
          try
          {
            ...
            if(debugLevel > 1)
              System.err.println("StorageSOPClassSCU(): affectedSOPClass = " + affectedSOPClass);
            if(debugLevel > 1)
              System.err.println("StorageSOPClassSCU(): affectedSOPInstance = " + affectedSOPInstance);
            if(debugLevel > 1)
              System.err.println("StorageSOPClassSCU(): inputTransferSyntax = " + inputTransferSyntax);
            ...
            byte presentationContextID =
               association.getSuitablePresentationContextID(affectedSOPClass);
            //int presentationContextID = association.getSuitablePresentationContextID(affectedSOPClass,TransferSyntax.Default);
            if(debugLevel > 0)
              System.err.println("Using context ID " + presentationContextID);
            String outputTransferSyntax =
            ...
            success =
               sendOneSOPInstance(association, affectedSOPClass, affectedSOPInstance,
               inputTransferSyntax, din,
        ...
        if(debugLevel > 0)
          System.err.println("Send " + fileName + " " + (success ? "succeeded" : "failed"));
      ...
      if(debugLevel > 0)
        System.err.println("StorageSOPClassSCU(): Finished sending all files nRemaining=" + nRemaining + " nCompleted=" + nCompleted + " nFailed=" + nFailed + " nWarning=" + nWarning);
    }
  }
    ...
    int nWarning = 0;
    {
      ...
      for(int i = 0; i < lists.length; ++i)
      {
        ...
        try
        {
          ...
          if(debugLevel > 0)
            System.err.println("Sending " + affectedSOPInstance);
          if(debugLevel > 1)
            System.err.println("StorageSOPClassSCU(): affectedSOPClass = " + affectedSOPClass);
//if (debugLevel > 1) System.err.println("StorageSOPClassSCU(): affectedSOPInstance = "+affectedSOPInstance);
        ...
        if(debugLevel > 0)
          System.err.println("Send " + affectedSOPInstance + " " + (success ? "succeeded" : "failed"));
