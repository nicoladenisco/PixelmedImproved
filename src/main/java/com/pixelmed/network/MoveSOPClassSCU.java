...
public class MoveSOPClassSCU extends SOPClass
{
  ...
  private static final String identString =
     "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/network/MoveSOPClassSCU.java,v 1.2 2011-08-10 18:14:19 nicola Exp $";
  /***/
  ...
  protected class CMoveResponseHandler extends CompositeResponseHandler
  {
    ...
    protected void evaluateStatusAndSetSuccess(AttributeList list)
    {
      ...
      if(debugLevel > 0)
        System.err.println("MoveSOPClassSCU.CMoveResponseHandler.evaluateStatusAndSetSuccess: status =0x" + Integer.toHexString(status));
      if(debugLevel > 0)
      ...
      if(debugLevel > 0)
        System.err.println("Move " + (status == 0xFF00 ? "pending"
                                      : (status == 0x0000 ? "success"
                                         : ("0x" + Integer.toHexString(status)))));

      int failedSuboperations =
         Attribute.getSingleIntegerValueOrDefault(list, TagFromName.NumberOfFailedSuboperations, 0);
     ...
     String affectedSOPClass, AttributeList identifier, int debugLevel) throws DicomNetworkException, DicomException, IOException
  {
    ...
    if(debugLevel > 0)
      System.err.println("MoveSOPClassSCU: Using context ID " + usePresentationContextID);

    byte cMoveRequestCommandMessage[] =
       new CMoveRequestCommandMessage(affectedSOPClass, moveDestination).getBytes();

    byte cMoveIdentifier[] =
       new IdentifierMessage(identifier, association.getTransferSyntaxForPresentationContextID(usePresentationContextID)).getBytes();

    if(debugLevel > 0)
      System.err.println("MoveSOPClassSCU: Identifier:\n" + identifier);
  ...
  public static void main(String arg[])
  {
    try
    {
      ...
      MoveSOPClassSCU moveSOPClassSCU =
         new MoveSOPClassSCU(arg[0], Integer.parseInt(arg[1]), arg[2], arg[3], arg[4], SOPClass.StudyRootQueryRetrieveInformationModelMove, identifier, 0/*debugLevel*/);
