 ...
 *
 * <p>
 * Supported sources of information include:</p>
 * <ul>
...
public class NetworkConfigurationFromMulticastDNS extends NetworkConfigurationSource
{
  private static final String identString
     = "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/network/NetworkConfigurationFromMulticastDNS.java,v 1.1.1.1 2011-05-21 10:08:47 nicola Exp $";
  ...
  private static final String AETTXTRecordPropertyNameAsLowerCase = AETTXTRecordPropertyName.toLowerCase();
  private static final String PrimaryDeviceTypeTXTRecordPropertyNameAsLowerCase
     = PrimaryDeviceTypeTXTRecordPropertyName.toLowerCase();
  ...
  protected class OurJmDNSServiceListener implements InvocationHandler
  {
       ...
       throws Throwable
    {
      if(method.getDeclaringClass() == javax.jmdns.ServiceListener.class)
      {
        ...
        if(debugLevel > 1)
          System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener.invoke(): methodName = " + methodName);
    ...
    public void serviceAdded(ServiceEvent event)
    {
      ...
      if(debugLevel > 1)
        System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service added   : Name = " + name);
      String type = event.getType();
      if(debugLevel > 1)
        System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service added   : Type = " + type);
      ServiceInfo info = event.getInfo();
      if(debugLevel > 1)
        System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service added   : Info = " + info);
      if(info == null)
      {
        if(debugLevel > 1)
    ...
    public void serviceRemoved(ServiceEvent event)
    {
      String name = event.getName();
      if(debugLevel > 1)
        System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service removed : Name = " + name);
      String type = event.getType();
      if(debugLevel > 1)
        System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service removed : Type = " + type);
      ServiceInfo info = event.getInfo();
      if(debugLevel > 1)
        System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service removed : Info = " + info);
    ...
    public void serviceResolved(ServiceEvent event)
    {
      String name = event.getName();
      if(debugLevel > 1)
        System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service resolved: Name = " + name);
      String type = event.getType();
      if(debugLevel > 1)
        System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service resolved: Type = " + type);
      ServiceInfo info = event.getInfo();
      if(debugLevel > 1)
        System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service resolved: Info = " + info);
      String aet = null;
      ...
      {
        if(debugLevel > 1)
          System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service resolved: Info getServer() = " + info.getServer());
        ...
        while(propertyNames.hasMoreElements())
        {
          ...
          if(propertyName != null)
          {
            ...
            {
              if(debugLevel > 1)
                System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service resolved: Unrecognized property name = " + propertyName + " value = " + propertyValue);
            }
          }
        }
      }
      ...
      if(debugLevel > 1)
        System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service resolved: hostname = " + hostname);
      if(debugLevel > 1)
        System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service resolved: port = " + port);
      if(debugLevel > 1)
        System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service resolved: aet = " + aet);
      if(debugLevel > 1)
        System.err.println("NetworkConfigurationFromMulticastDNS.OurJmDNSServiceListener: Service resolved: primaryDeviceType = " + primaryDeviceType);
         ...
         && port != -1)
      {
        ...
        ApplicationEntity ae
           = new ApplicationEntity(aet, presentationAddress, null/*queryModel*/, primaryDeviceType);
        try
        {
  ...
  /**
   * <p>
   * Start DNS Self-Discovery, if possible.</p>
   *
   * <p>
   * Requires <code>javax.jmdns</code> package to be in class path.</p>
   *
  ...
  public void activateDiscovery(int refreshInterval)
  {
    ...
    try
    {
      ...
      ServiceListener listener
         = (ServiceListener) Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(),
            new Class[]
            {
  ...
  public void deActivateDiscovery()
  {
    ...
    if(jmDNS != null)
    {
      try
      {
        ...
        // needed, since otherwise application will not exit when main thread finished
        jmDNS.close();
      }
      catch(Exception ex)
      {
  ...
  /**
   * <p>
   * Construct an instance capable of handling dynamic configuration information but do not start anything yet.</p>
   *
   * @param	debugLevel
   */
...
//System.err.println("NetworkConfigurationFromMulticastDNS(): debugLevel = "+debugLevel);
  ...
  /**
   * <p>
   * Unregister all services that have been registered.</p>
   *
   */
  ...
  /**
   * <p>
   * Register a DICOM service on the local host.</p>
   *
  ...
  public void registerDicomService(String calledApplicationEntityTitle, int port, String primaryDeviceType)
  {
    ...
    if(jmDNS != null)
    {
      ...
      try
      {
        ...
        Class[] argTypes =
        {
          String.class, String.class, java.lang.Integer.TYPE, java.lang.Integer.TYPE,
          java.lang.Integer.TYPE, Hashtable.class
        };
        Object[] argValues =
        {
          DICOMServiceName, calledApplicationEntityTitle, port, 0/*weight*/, 0/*priority*/,
          properties
        };
      ...
      {	// may be ClassNotFoundException,NoSuchMethodException,InstantiationException,IOException
        e.printStackTrace(System.err);
      }
    }
  }
  ...
  /**
   * <p>
   * Register a WADO service on the local host.</p>
   *
...
//System.err.println("NetworkConfigurationFromMulticastDNS.registerWADOService():");
    if(jmDNS != null)
    {
      ...
      try
      {
        ...
        Object[] argValues =
        {
          WADOServiceName, instanceName, port, 0/*weight*/, 0/*priority*/, properties
        };
        ServiceInfo info = (ServiceInfo) (classToUse.getConstructor(argTypes).newInstance(argValues));
        if(debugLevel > 1)
          System.err.println("NetworkConfigurationFromMulticastDNS.registerWADOService(): info = " + info);
        jmDNS.registerService(info);
      }
      catch(Exception e)
      {	// may be ClassNotFoundException,NoSuchMethodException,InstantiationException,IOException
  ...
  /**
   * <p>
   * Utility that activates a dynamic configuration listener and dumps its contents periodically.</p>
   *
   * <p>
   * Additionally, will register a DICOM service on the local machine, if parameters of that service are supplied.</p>
   *
  ...
  public static void main(String arg[])
  {
    ...
    if(arg.length > 1)
    {
      networkConfiguration.registerDicomService(arg[0], Integer.parseInt(arg[1]), arg.length > 2
                                                                                     ? arg[2] : null);
    }
