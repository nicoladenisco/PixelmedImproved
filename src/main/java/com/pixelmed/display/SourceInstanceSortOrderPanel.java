 ...
 * @author	dclunie
 */
abstract public class SourceInstanceSortOrderPanel extends JPanel
{
  private static final String identString =
     "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/display/SourceInstanceSortOrderPanel.java,v 1.3 2011-05-21 18:44:39 nicola Exp $";
  /***/
  ...
  public class MapOfIndexValuesToFrameNumberEntry implements Comparable
  {
     ...
     * @param	frameNumber
     */
    public MapOfIndexValuesToFrameNumberEntry(int[] values, int frameNumber)
    {
      this.values = values;
  ...
  /***/
  public class MapOfIndexValuesToFrameNumber
  {
    /***/
    protected int nFrames;
    /***/
    protected MapOfIndexValuesToFrameNumberEntry[] map;

    /**
     * @param	list
     */
    public MapOfIndexValuesToFrameNumber(AttributeList list)
    {
      nFrames = 0;
      map = null;
      try
      {
        SequenceAttribute aPerFrameFunctionalGroupsSequence =
           (SequenceAttribute) list.get(TagFromName.PerFrameFunctionalGroupsSequence);
        if(aPerFrameFunctionalGroupsSequence != null)
        {
          ...
          while(pfitems.hasNext())
          {
            ...
            if(flist != null)
            {
              SequenceAttribute aFrameContentSequence =
                 (SequenceAttribute) flist.get(TagFromName.FrameContentSequence);
              if(aFrameContentSequence != null && aFrameContentSequence.getNumberOfItems() >= 1)
              { 		// should be 1
            ...
            {
              if(map == null)
                map = new MapOfIndexValuesToFrameNumberEntry[nFrames];
              map[frameNumber] =
                 new MapOfIndexValuesToFrameNumberEntry(vDimensionIndexValues, frameNumber);
            }
            ++frameNumber;
          }
        }
...
//System.err.println("sortOrder["+i+"]"+array[i]);
  ...
  protected Vector buildListOfDimensionsFromAttributeList(AttributeList list)
  {
    ...
    try
    {
      SequenceAttribute aDimensionIndexSequence =
         (SequenceAttribute) list.get(TagFromName.DimensionIndexSequence);
      if(aDimensionIndexSequence != null)
      {
        ...
        while(items.hasNext())
        {
          ...
          if(ilist != null)
          {
            ...
            String name =
               Attribute.getSingleStringValueOrNull(ilist, TagFromName.DimensionDescriptionLabel);
            if(name == null || name.length() == 0)
            {
              // try using tag name from dictionary if known
              AttributeTagAttribute aDimensionIndexPointer =
                 (AttributeTagAttribute) ilist.get(TagFromName.DimensionIndexPointer);
...
//System.err.println("SourceImageSortOrderPanel.SortActionListener.actionPerformed()");
      ...
      try
      {
        ...
        {
          useSortOrder =
             new MapOfIndexValuesToFrameNumber(currentSrcInstanceAttributeList).getFrameNumberArray();	// null if no dimension index values
        }
