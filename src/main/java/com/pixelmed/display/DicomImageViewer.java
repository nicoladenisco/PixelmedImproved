...
import org.commonlib5.utils.OSXAdapter;
 ...
 *
 * <p>
 * It supports a local database of DICOM objects, as well as the ability to
   ...
   KeyListener, MouseListener
{
  ...
  private static final String identString
     = "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/display/DicomImageViewer.java,v 1.4 2011-10-03 12:54:44 nicola Exp $";
  ...
  private static final String propertyName_DicomCurrentlySelectedStorageTargetAE
  ...
  private static LongOptExt[] longopts = null;
  public static String lookfClassName = null;
  public static boolean debugMode = false;
  public static boolean kioskMode = false;
  public static File fileSetup = null;
  public static boolean flagMaximize = false;
  public static boolean flagFullScreen = false;
  public static int fullScreenDevice = -1;
  public static File acroread = null;
  ...
  { 					// override ApplicationFrame's method to exit on closing
...
//System.err.println("mouseEntered event"+e);
  ...
  private void createTableOfCurrentAttributesForCurrentFrameBrowser()
  {
    ...
    modelOfCurrentAttributesForCurrentFrameBrowser
       = new AttributeListFunctionalGroupsTableModelOneFrame(null, null, excludeList);	// list of attributes set later when image selected
    tableOfCurrentAttributesForCurrentFrameBrowser
       = new AttributeListTableBrowser(modelOfCurrentAttributesForCurrentFrameBrowser);
  ...
  private void createTableOfCurrentAttributesForAllFramesBrowser()
  {
    ...
    modelOfCurrentAttributesForAllFramesBrowser
       = new AttributeListFunctionalGroupsTableModelAllFrames(null, null, excludeList);	// list of attributes set later when image selected
    tableOfCurrentAttributesForAllFramesBrowser
       = new AttributeListTableBrowser(modelOfCurrentAttributesForAllFramesBrowser);
...
//System.err.println("DicomImageViewer.OurBrowserPaneChangeListener.changed() to SR");
  ...
  private GeometryOfVolume getNewGeometryOfVolume(AttributeList attributeList)
  {
    ...
    {	// NoClassDefFoundError may be thrown if no vecmath support available, which is an Error, not an Exception
...
//System.err.println("loadMultiPanelWithSpectra:");
    ...
    if(sSpectra != null)
    {
      ...
      SpectraPanel sp
         = new SpectraPanel(spectra, sSpectra.getRows(), sSpectra.getColumns(), sSpectra.getMinimum(), sSpectra.getMaximum(),
  ...
  private void setCurrentRemoteQueryInformationModel(String remoteAEForQuery, JTabbedPane browserPane, int tabNumberOfRemoteInBrowserPane)
  {
    ...
    if(remoteAEForQuery != null && remoteAEForQuery.length() > 0 && networkApplicationProperties != null && networkApplicationInformation != null)
    {
      try
      {
        ...
        String queryCalledAETitle
           = networkApplicationInformation.getApplicationEntityTitleFromLocalName(remoteAEForQuery);
        PresentationAddress presentationAddress
           = networkApplicationInformation.getApplicationEntityMap().getPresentationAddress(queryCalledAETitle);
        ...
        if(NetworkApplicationProperties.isStudyRootQueryModel(queryModel) || queryModel == null)
        {
          currentRemoteQueryInformationModel
             = new StudyRootQueryInformationModel(queryHost, queryPort, queryCalledAETitle, queryCallingAETitle, queryDebugLevel);
      ...
      {		// if an AE's property has no value, or model not supported
        e.printStackTrace(System.err);
      }
    }
    if(browserPane != null)
...
//System.err.println("DicomImageViewer.getCurrentRemoteQueryFilter(): now "+currentRemoteQueryFilter);
  ...
  {
    return (currentFilePathSelections != null && currentFilePathSelections.size() > 0)
              ? (String) (currentFilePathSelections.get(0)) : null;
  }
  ...
  private void setCurrentDatabaseTreeRecordSelection(DatabaseTreeRecord r)
  {
    currentDatabaseTreeRecordSelection = r;
  }

  private DatabaseTreeRecord getCurrentDatabaseTreeRecordSelection()
  {
    return currentDatabaseTreeRecordSelection;
  }
  ...
  private class OurDicomDirectoryBrowser extends DicomDirectoryBrowser
  {
     ...
     */
    public OurDicomDirectoryBrowser(AttributeList list)
...
//System.err.println("DicomImageViewer.OurDatabaseTreeBrowser.doSomethingMoreWithWhateverWasSelected():");
  ...
  private class OurQueryTreeBrowser extends QueryTreeBrowser
  {
     ...
     * @exception	DicomException
     */
    OurQueryTreeBrowser(QueryInformationModel q, QueryTreeModel m, JScrollPane treeBrowserScrollPane, JScrollPane attributeBrowserScrollPane)
       throws DicomException
    {
...
//System.err.println("DicomImageViewer.loadDicomFileOrDirectory(): SOPClass.isStructuredReport or AttributeList.isSRDocument()");
  ...
  private final TreeMap getImageListMappedToFilenamesForReferenceOrBackground(AttributeList referencedFromList, boolean requireSameImageOrientationPatient)
  {
    ...
    String frameOfReferenceUID
       = Attribute.getSingleStringValueOrNull(referencedFromList, TagFromName.FrameOfReferenceUID);
    double[] wantedImageOrientationPatient
       = GeometryOfVolumeFromAttributeList.getImageOrientationPatientFromAttributeList(referencedFromList);
    if(frameOfReferenceUID != null)
    {
      if(databaseInformationModel != null)
      {
        try
        {
          ...
          ArrayList returnedRecords
             = databaseInformationModel.findAllAttributeValuesForAllRecordsForThisInformationEntityWithMatchingParent(
...
//System.err.println(referencedList);
            if(referencedList != null)
            {
              ...
              String sopInstanceUID
                 = Attribute.getSingleStringValueOrNull(referencedList, TagFromName.ReferencedSOPInstanceUIDInFile);
              double[] imageOrientationPatient
                 = Attribute.getDoubleValues(referencedList, TagFromName.ImageOrientationPatient);
...
//System.err.println("DicomImageViewer.showInputDialogToSelectNetworkTargetByLocalApplicationEntityName(): got localNamesOfRemoteAEs");
        ...
        ae
           = (String) JOptionPane.showInputDialog(getContentPane(), question, buttonText, JOptionPane.QUESTION_MESSAGE, null, sta, ae);
      }
    }
    return ae;
  }
...
//System.err.println("QueryFilterActionListener.actionPerformed()");
  ...
  private class QueryRetrieveActionListener implements ActionListener
  {
    ...
    public void actionPerformed(ActionEvent event)
    {
      Cursor was = getCursor();
      setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
      if(getCurrentRemoteQuerySelectionLevel() == null)
      {	// they have selected the root of the tree
...
//System.err.println("DicomImageViewer.purgeFilesAndDatabaseInformation(): removeFromParent instance level "+databaseSelection);
  ...
  private class DatabaseImportFromFilesActionListener implements ActionListener
  {
    ...
    {
      this.importer
         = new DatabaseMediaImporter(null/*initial path*/, savedImagesFolder, storedFilePathStrategy, databaseInformationModel,/*null*/ new OurMessageLogger());
    }
...
//System.err.println("DicomImageViewer.DatabaseImportFromFilesActionListener.actionPerformed(): caling importer.choosePathAndImportDicomFiles()");
   ...
   * @exception	DicomException
   */
  private void copyFileAndImportToDatabase(AttributeList list, String fileName)
     throws DicomException, IOException
  {
    ...
    String localCopyFileName
       = storedFilePathStrategy.makeReliableStoredFilePathWithFoldersCreated(null, savedImagesFolder, sopInstanceUID).getPath();
...
//System.err.println("NetworkSendCurrentSelectionActionListener.actionPerformed(): CoercionModel="+coercionModel);
        ...
        if(ae != null && networkApplicationProperties != null)
        {
          try
          {
            ...
            PresentationAddress presentationAddress
               = networkApplicationInformation.getApplicationEntityMap().getPresentationAddress(calledAETitle);
...
//System.err.println("NetworkSendCurrentSelectionActionListener.actionPerformed(): dicomFileName="+dicomFileName);
          ...
          {		// if an AE's property has no value
            e.printStackTrace(System.err);
          }
        }
  ...
  public void registerForMacOSXEvents()
  {
    if(System.getProperty("os.name").toLowerCase().startsWith("mac os x"))
    {
      try
      {
        ...
        OSXAdapter.setQuitHandler(this, getClass().getDeclaredMethod("quit"));		// need this, else won't quite from X or Cmd-Q any more, once any events registered
        //OSXAdapter.setAboutHandler(this, getClass().getDeclaredMethod("about", (Class[])null));
        //OSXAdapter.setPreferencesHandler(this, getClass().getDeclaredMethod("preferences", (Class[])null));
        OSXAdapter.setFileHandler(this, getClass().getDeclaredMethod("osxFileHandler", new Class[]
        {
          String.class
        }));
      }
...
//System.err.println("Starting up database ...");
      ...
      try
      {
        databaseInformationModel
           = new PatientStudySeriesConcatenationInstanceModel(makePathToFileInUsersHomeDirectory(databaseApplicationProperties.getDatabaseFileName()), databaseApplicationProperties.getDatabaseServerName());
    ...
    if(networkApplicationProperties != null)
    {
      try
      {
        ...
        new Thread(new StorageSOPClassSCPDispatcher(port, calledAETitle, savedImagesFolder, storedFilePathStrategy, new OurReceivedObjectHandler(),
           databaseInformationModel == null ? null
              : databaseInformationModel.getQueryResponseGeneratorFactory(queryDebugLevel),
           databaseInformationModel == null ? null
              : databaseInformationModel.getRetrieveResponseGeneratorFactory(queryDebugLevel),
           networkApplicationInformation,
...
//System.err.println("Building GUI ...");
    ...
    displayButtonsPanel.add(displayValidateButton);
    {
      ...
      displayControlsSubPanel.add(displaySortPanel, BorderLayout.NORTH);
      {
        ...
        sourceImageVOILUTSelectorPanel
           = new SourceImageVOILUTSelectorPanel(null/* Apply to all contexts, not just WellKnownContext.MAINPANEL*/);
        displayControlsSubSubPanel.add(sourceImageVOILUTSelectorPanel);

        sourceImageGraphicDisplaySelectorPanel
           = new SourceImageGraphicDisplaySelectorPanel(null/* Apply to all contexts, not just WellKnownContext.MAINPANEL*/);
    ...
    }
    {
      ...
      JScrollPane scrollingDisplayListOfPossibleReferenceImages
         = new JScrollPane(displayListOfPossibleReferenceImagesForImages);
    ...
    spectroscopyControlsPanel.add(spectroscopySortPanel, BorderLayout.CENTER);
    {
      ...
      spectroscopyControlsPanel.add(spectroscopyBackgroundAndReferenceGroupPanel, BorderLayout.SOUTH);
      {
        ...
        JScrollPane scrollingDisplayListOfPossibleBackgroundImages
           = new JScrollPane(displayListOfPossibleBackgroundImagesForSpectra);
      ...
      }
      {
        ...
        JScrollPane scrollingDisplayListOfPossibleReferenceImages
           = new JScrollPane(displayListOfPossibleReferenceImagesForSpectra);
...
//System.err.println("Loading DICOM file or chooser ...");
    ...
    {		// not a very robust flag for no DICOMDIR or input file, but it will do
...
//System.err.println("Building action listeners ...");
    ...
    ImportCurrentlyDisplayedInstanceToDatabaseActionListener importCurrentlyDisplayedInstanceToDatabaseActionListener
       = new ImportCurrentlyDisplayedInstanceToDatabaseActionListener();
...
//System.err.println("Building ValidateCurrentlyDisplayedImageActionListener ...");
    ...
    JSplitPane browserAndMultiPaneAndCurrentAttributes
       = new JSplitPane(JSplitPane.VERTICAL_SPLIT, browserAndMultiPane, scrollPaneOfCurrentAttributes);
    ...
    // attiva la massimazione della finestra se richiesta
    if(flagMaximize)
    {
    ...
    else
    {
      ...
      if(flagFullScreen || kioskMode || (fullScreen != null && fullScreen.equals("true")))
      {
        GraphicsEnvironment ge = GraphicsEnvironment.getLocalGraphicsEnvironment();
        GraphicsDevice[] devices = ge.getScreenDevices();

        if(fullScreenDevice >= devices.length)
        {
          System.out.println("Undefined full screen device " + fullScreenDevice
             + "; use normal window.");
        }
        else
        {
          GraphicsDevice gs = fullScreenDevice == -1
                                 ? ge.getDefaultScreenDevice() : devices[fullScreenDevice];

          if(gs != null && gs.isFullScreenSupported())
          {
            setUndecorated(true);
            setResizable(false);
          ...
          {
            System.out.println("The device dont support full screen mode; use normal window.");
          }
        }
      }
  ...
  /**
   * <p>
   * The method to invoke the application.</p>
   *
  ...
  public static void main(String arg[])
  {
    ...
    if(System.getProperty("mrj.version") != null)
    {
      ...
      // You might want to use this property during development of full-screen Java applications.
      // This is strictly a developer option.
      // Must be done before creating components
      System.setProperty("apple.awt.fakefullscreen", "true");
    }

    longopts = new LongOptExt[]
    {
      new LongOptExt("help", LongOpt.NO_ARGUMENT, null, 'h', "visualizza questo messaggio ed esce"),
      new LongOptExt("debug", LongOpt.NO_ARGUMENT, null, 'D', "abilita output per debugging"),
      new LongOptExt("laf", LongOpt.REQUIRED_ARGUMENT, null, 'l', "imposta look and feel"),
      new LongOptExt("config", LongOpt.REQUIRED_ARGUMENT, null, 'c', "imposta file di configurazione"),
      new LongOptExt("maximize", LongOpt.NO_ARGUMENT, null, 'M', "apre finestra massimizzata"),
      new LongOptExt("full-screen", LongOpt.NO_ARGUMENT, null, 'S', "modalita full screen"),
      new LongOptExt("full-screen-device", LongOpt.REQUIRED_ARGUMENT, null, 's', "modalita full screen con specifica della device"),
      new LongOptExt("kiosk-mode", LongOpt.NO_ARGUMENT, null, 'k', "modalita chiosco"),
      new LongOptExt("acroread", LongOpt.REQUIRED_ARGUMENT, null, 'A', "specifica programma per visualizzazione pdf"),
    };

    String optString = LongOptExt.getOptstring(longopts);
    Getopt g = new Getopt("DicomViewer", arg, optString, longopts);
    ...
    while((c = g.getopt()) != -1)
    {
      switch(c)
      {
        ...
        case 'l':
          ...
          break;
        case 'c':
          fileSetup = new File(g.getOptarg());
          break;
        case 'M':
          flagMaximize = true;
          break;
        case 'S':
          ...
          break;
        case 'A':
          acroread = new File(g.getOptarg());
          if(!acroread.exists())
            acroread = null;
          break;
        case 's':
          fullScreenDevice = Integer.parseInt(g.getOptarg());
          flagFullScreen = true;
          break;
      }
    }
    ...
    try
    {
      if(lookfClassName == null)
      {
        // Tell the UIManager to use the platform look and feel (non per Max OSX)
        UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
      }
      else
      {
        if(lookfClassName.equals("nimrod"))
          lookfClassName = "com.nilo.plaf.nimrod.NimRODLookAndFeel";

        // usa look and feel specificato sulla linea di comando
        UIManager.setLookAndFeel(lookfClassName);
      }
    }
    catch(Exception e)
    {
    ...
    String dicomFileName = null;
    if(g.getOptind() < arg.length)
    {
      dicomFileName = arg[g.getOptind()];
    }
  ...
  {
    System.out.println(
       "DicomImageViewer ver. 1.0\n"
       + "Visualizzatore di immagini DICOM.\n"
       + "modo d'uso:\n"
       + "  viewer [options]\n");
