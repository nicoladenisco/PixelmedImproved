...
/**
 * <p>An abstract class to describe the spatial geometry of an entire volume of contiguous cross-sectional image slices.</p>
 *
 ...
 * @author	dclunie
 */
abstract public class GeometryOfVolume {

	private static final String identString = "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/geometry/GeometryOfVolume.java,v 1.1.1.1 2011-05-21 10:08:47 nicola Exp $";
	 ...
	 * return the volume coordinates (column and row and frame offset).</p>
	 *
	 * @param	offsets		an array in which to return the column and row and frame offsets from the top left hand corner of the volume
	 * @param	location	the x, y and z location in 3D space
	 */
	...
	/*
	location[0] = tlhcArray[0] + row*columnArray[0]*voxelSpacingArray[0] + column*rowArray[0]*voxelSpacingArray[1] + frame*normalArray[0]*voxelSpacingArray[2];
	location[1] = tlhcArray[1] + row*columnArray[1]*voxelSpacingArray[0] + column*rowArray[1]*voxelSpacingArray[1] + frame*normalArray[1]*voxelSpacingArray[2];
	location[2] = tlhcArray[2] + row*columnArray[2]*voxelSpacingArray[0] + column*rowArray[2]*voxelSpacingArray[1] + frame*normalArray[2]*voxelSpacingArray[2];
	
	
	row = (location[0] - tlhcArray[0] - column*rowArray[0]*voxelSpacingArray[1] - frame*normalArray[0]*voxelSpacingArray[2]) / (columnArray[0]*voxelSpacingArray[0]);
	row = (location[1] - tlhcArray[1] - column*rowArray[1]*voxelSpacingArray[1] - frame*normalArray[1]*voxelSpacingArray[2]) / (columnArray[1]*voxelSpacingArray[0]);
	row = (location[2] - tlhcArray[2] - column*rowArray[2]*voxelSpacingArray[1] - frame*normalArray[2]*voxelSpacingArray[2]) / (columnArray[2]*voxelSpacingArray[0]);
	
	column = ((location[0] - tlhcArray[0] - frame*normalArray[0]*voxelSpacingArray[2]) * columnArray[1]*voxelSpacingArray[0]
	- (location[1] - tlhcArray[1] - frame*normalArray[1]*voxelSpacingArray[2]) * columnArray[0]*voxelSpacingArray[0])
	/ (rowArray[0]*voxelSpacingArray[1] * columnArray[1]*voxelSpacingArray[0] - rowArray[1]*voxelSpacingArray[1]* columnArray[0]*voxelSpacingArray[0])
	
	column
	 = ((location[2] - tlhcArray[2] - frame*normalArray[2]*voxelSpacingArray[2]) * columnArray[1]*voxelSpacingArray[0]
	   - (location[1] - tlhcArray[1] - frame*normalArray[1]*voxelSpacingArray[2]) * columnArray[2]*voxelSpacingArray[0])
	/ (rowArray[2]*voxelSpacingArray[1] * columnArray[1]*voxelSpacingArray[0] - rowArray[1]*voxelSpacingArray[1] * columnArray[2]*voxelSpacingArray[0])

	*/

		double frame = 
		(
		   ((location[0] - tlhcArray[0])*columnArray[1]*voxelSpacingArray[0] - (location[1] - tlhcArray[1])*columnArray[0]*voxelSpacingArray[0])
		     * (rowArray[2]*voxelSpacingArray[1]*columnArray[1]*voxelSpacingArray[0] - rowArray[1]*voxelSpacingArray[1]*columnArray[2]*voxelSpacingArray[0])
		 - ((location[2] - tlhcArray[2])*columnArray[1]*voxelSpacingArray[0] - (location[1] - tlhcArray[1])*columnArray[2]*voxelSpacingArray[0])
		     * (rowArray[0]*voxelSpacingArray[1]*columnArray[1]*voxelSpacingArray[0] - rowArray[1]*voxelSpacingArray[1]*columnArray[0]*voxelSpacingArray[0])
		)
		/
		(     (normalArray[0]*voxelSpacingArray[2]*columnArray[1]*voxelSpacingArray[0] + normalArray[1]*voxelSpacingArray[2]*columnArray[0]*voxelSpacingArray[0])
		    * (   rowArray[2]*voxelSpacingArray[1]*columnArray[1]*voxelSpacingArray[0] -    rowArray[1]*voxelSpacingArray[1]*columnArray[2]*voxelSpacingArray[0])
		  -   (normalArray[2]*voxelSpacingArray[2]*columnArray[1]*voxelSpacingArray[0] + normalArray[1]*voxelSpacingArray[2]*columnArray[2]*voxelSpacingArray[0])
		    * (   rowArray[0]*voxelSpacingArray[1]*columnArray[1]*voxelSpacingArray[0] -    rowArray[1]*voxelSpacingArray[1]*columnArray[0]*voxelSpacingArray[0])
		);

		double column =
		(   (location[0] - tlhcArray[0] - frame*normalArray[0]*voxelSpacingArray[2]) * columnArray[1]*voxelSpacingArray[0]
		  - (location[1] - tlhcArray[1] - frame*normalArray[1]*voxelSpacingArray[2]) * columnArray[0]*voxelSpacingArray[0]
		)
		/ (rowArray[0]*voxelSpacingArray[1]*columnArray[1]*voxelSpacingArray[0] - rowArray[1]*voxelSpacingArray[1]*columnArray[0]*voxelSpacingArray[0])
		;
