...
public class MultiModalitySimulator
{
  private static final String identString =
     "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/apps/MultiModalitySimulator.java,v 1.2 2011-06-13 12:34:14 nicola Exp $";

  protected static String[] modalities =
  {
    "CT", "MR", "DX", "CR", "US", "NM", "XA"
  };
  ...
  {
    return sleepIntervalForModality.get(modality).intValue() * 60 * 1000;
  }
  ...
  protected class OurMultipleInstanceTransferStatusHandler extends MultipleInstanceTransferStatusHandlerWithFileName
  {
    ...
    {
      if(debugLevel > 2)
        System.err.println("Remaining " + nRemaining + ", completed " + nCompleted + ", failed " + nFailed + ", warning " + nWarning);
      if(success)
      {
        if(debugLevel > 1)
          System.err.println("Sent " + fileName);
      }
      else
      {
  ...
  {
    if(debugLevel > 2)
      System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.findFilesToSend(): checking " + ie + " " + localPrimaryKeyValue);
    if(ie == InformationEntity.INSTANCE)
    {
      Map<String, String> record =
         databaseInformationModel.findAllAttributeValuesForSelectedRecord(InformationEntity.INSTANCE, localPrimaryKeyValue);
      ...
      if(debugLevel > 2)
        System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.findFilesToSend(): added file = " + fileName);
    }
    else
    {
      InformationEntity childIE = databaseInformationModel.getChildTypeForParent(ie);
      if(debugLevel > 2)
        System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.findFilesToSend(): childIE is " + childIE);

      List<Map<String, String>> returnedRecords =
         databaseInformationModel.findAllAttributeValuesForAllRecordsForThisInformationEntityWithSpecifiedParent(childIE, localPrimaryKeyValue);
      ...
      while(childIE != null && returnedRecords == null || returnedRecords.size() == 0)
      {
        ...
        if(debugLevel > 2)
          System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.findFilesToSend(): empty so descending to next childIE " + childIE);
        returnedRecords =
           databaseInformationModel.findAllAttributeValuesForAllRecordsForThisInformationEntityWithSpecifiedParent(childIE, localPrimaryKeyValue);
      }
      ...
      if(returnedRecords != null)
      {
        ...
        {
          String childLocalPrimaryKeyValue =
             record.get(databaseInformationModel.getLocalPrimaryKeyColumnName(childIE));
  ...
  protected SetOfDicomFiles generateSyntheticStudyFromOriginal(List<String> originalDicomFileNames,
     String modality, String aeTitleForMetaInformation,
     String patientName, String patientID, String studyID, String accessionNumber)
     throws DicomException, IOException
  {
    if(debugLevel > 0)
      System.err.println("MultiModalitySimulator.generateSyntheticStudyFromOriginal(): generating modality=" + modality + " patientName=" + patientName + " patientID=" + patientID + " studyID=" + studyID + " accessionNumber=" + accessionNumber);
    ...
    if(originalDicomFileNames != null)
    {
      for(String originalDicomFileName : originalDicomFileNames)
      {
        ...
        {
          if(debugLevel > 1)
            System.err.println("MultiModalitySimulator.generateSyntheticStudyFromOriginal(): doing file " + originalDicomFileName);
          ...
          }
          {
            ...
            String currentTime =
               new java.text.SimpleDateFormat("HHmmss.SSS").format(currentDateTime);
            {
          ...
          if(debugLevel > 1)
            System.err.println("MultiModalitySimulator.generateSyntheticStudyFromOriginal(): synthetic file " + syntheticFileName);
        }
      }
    }
  ...
  protected class SpecificModalitySimulator implements Runnable
  {
    ...
    SpecificModalitySimulator(String modality, Set<String> localPrimaryKeysOfAllStudiesOfThisModality)
    {
      ...
      this.localPrimaryKeysOfAllStudiesOfThisModality =
         localPrimaryKeysOfAllStudiesOfThisModality.toArray(new String[numberOfStudies]);
    ...
    public void run()
    {
      if(numberOfStudies > 0)
      {
        ...
        while(!interrupted)
        {
          ...
          if(debugLevel > 1)
            System.err.println("MultiModalitySimulator.SpecificModalitySimulator(): Selected " + modality + " " + studySelected);
          
          String localPrimaryKeyOfSelectedStudy =
             localPrimaryKeysOfAllStudiesOfThisModality[studySelected];

          if(debugLevel > 1)
            System.err.println("MultiModalitySimulator.SpecificModalitySimulator(): Selected " + modality + " " + studySelected + " " + localPrimaryKeyOfSelectedStudy);
          ...
          try
          {
            ...
            SetOfDicomFiles syntheticDicomFiles =
               generateSyntheticStudyFromOriginal(originalFileNames, modality, modality/*aeTitleForMetaInformation*/, patientName, patientID, studyID, accessionNumber);
  ...
  public MultiModalitySimulator(String hostname, int port, String calledAETitle, String databaseFileName, int debugLevel) throws DicomException
  {
    ...
    if(debugLevel > 0)
      System.err.println("MultiModalitySimulator(): Opening database ...");
    ...
    seriesLocalParentReferenceColumnName =
       databaseInformationModel.getLocalParentReferenceColumnName(InformationEntity.SERIES);
    localFileNameColumnName =
       databaseInformationModel.getLocalFileNameColumnName(InformationEntity.INSTANCE);
    modalityColumnName =
       databaseInformationModel.getDatabaseColumnNameFromDicomTag(TagFromName.Modality);
    ...
    if(debugLevel > 0)
      System.err.println("MultiModalitySimulator(): Building indexes of modality studies from database ...");
    Map<String, Set<String>> localPrimaryKeysOfStudiesByModality =
    ...
    for(String modality : modalities)
    {
      ...
      List<Map<String, String>> allSeriesOfThisModality =
         databaseInformationModel.findAllAttributeValuesForAllRecordsForThisInformationEntityWithSpecifiedKeyValue(
         InformationEntity.SERIES, modalityColumnName, modality);
    ...
    if(debugLevel > 0)
      System.err.println("MultiModalitySimulator(): ready to begin simulation ...");

    for(String modality : modalities)
    {
      Set<String> localPrimaryKeysOfAllStudiesOfThisModality =
         localPrimaryKeysOfStudiesByModality.get(modality);
      ...
      {
        if(debugLevel > 0)
          System.err.println("MultiModalitySimulator(): no " + modality + " studies");
      }
    }
  }
