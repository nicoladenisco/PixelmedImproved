...
public class DoseReporterWithLegacyOCRAndAutoSendToRegistry {
	...
	private static final String identString = "@(#) $Header: /var/cvs-rep/pmedim/src/com/pixelmed/apps/DoseReporterWithLegacyOCRAndAutoSendToRegistry.java,v 1.2 2011-06-16 11:23:00 nicola Exp $";
...
if (applicationDebugLevel > 1) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.deidentifyFile(): deidentified file is "+deidentifiedFileName);
	...
	protected void sendFileToRegistry(CandidateFile candidateFile) throws DicomException, IOException, NoSuchAlgorithmException, Exception {
		...
		String deidentifiedFileName = deidentifyFile(candidateFile.fileName);
...
if (applicationDebugLevel > 0) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.sendFileToRegistry(): successful send of deidentified version of "+candidateFile.fileName);
...
if (applicationDebugLevel > 1) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.sendFileToRegistry(): deleting deidentified file "+deidentifiedFileName);
			if (!new File(deidentifiedFileName).delete()) {
				// Note that throwing exception at this point would prevent sending of rest of files by caller ... better to let files leak
				//throw new DicomException("Failed to delete deidentified file that we sent to registry "+deidentifiedFileName);
				System.err.println("Failed to delete deidentified file that we sent to registry "+deidentifiedFileName);
			}
		}
	}
...
if (applicationDebugLevel > 1) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.deleteFilesAndDatabaseRecords(): deleting file "+candidateFile.fileName);
			if (!new File(candidateFile.fileName).delete()) {
				throw new DicomException("Failed to delete deidentified file that we sent to registry "+candidateFile.fileName);
			}
...
if (applicationDebugLevel > 2) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.CandidateFile(): sourceApplicationEntityTitle = "+sourceApplicationEntityTitle);
			...
			instanceHasBeenSent = instanceHasBeenSentValue != null && instanceHasBeenSentValue.toUpperCase().equals("TRUE");
if (applicationDebugLevel > 2) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.CandidateFile(): instanceHasBeenSent = "+instanceHasBeenSent);
			...
			isRadiationDoseSR = isRadiationDoseSRValue != null && isRadiationDoseSRValue.toUpperCase().equals("TRUE");
if (applicationDebugLevel > 2) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.CandidateFile(): isRadiationDoseSR = "+isRadiationDoseSR);
	...
	protected long findCandidateFilesToSendToRegistry(InformationEntity ie,String localPrimaryKeyValue,Set<CandidateFile> setOfOriginalSRFiles,Set<CandidateFile> setOfLegacyOCRSRFiles,long mostRecentInsertionTime) throws DicomException {
...
if (applicationDebugLevel > 0) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.findCandidateFilesToSendToRegistry(): is not RDSR fileName = "+candidateFile.fileName);
			}
		}
		else {
...
if (applicationDebugLevel > 2) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.findCandidateFilesToSendToRegistry(): mostRecentInsertionTime before recursing = "+mostRecentInsertionTime);
				mostRecentInsertionTime = findCandidateFilesToSendToRegistry(childIE,childLocalPrimaryKeyValue,setOfOriginalSRFiles,setOfLegacyOCRSRFiles,mostRecentInsertionTime);
if (applicationDebugLevel > 2) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.findCandidateFilesToSendToRegistry(): mostRecentInsertionTime after recursing = "+mostRecentInsertionTime);
	...
	protected boolean findSuitableSRFilesAndSendThemToRegistry(String studyLocalPrimaryKeyValue) throws DicomException, IOException, NoSuchAlgorithmException, Exception {
		...
		long mostRecentInsertionTime = findCandidateFilesToSendToRegistry(InformationEntity.STUDY,studyLocalPrimaryKeyValue,setOfOriginalSRFiles,setOfLegacyOCRSRFiles,0l);
...
if (applicationDebugLevel > 0) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.sendFilesToRegistry(): processing, since old enough");
			...
			// prefer to send original manufacturer's files, if any, else ours, if any
			if (setOfOriginalSRFiles.isEmpty()) {
				if (!setOfLegacyOCRSRFiles.isEmpty()) {
if (applicationDebugLevel > 1) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.sendFilesToRegistry(): sending our legacy extracted SR files");
					sendFilesToRegistry(setOfLegacyOCRSRFiles);
				}
			}
			else {
if (applicationDebugLevel > 1) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.sendFilesToRegistry(): sending manufacturer's SR files");
				sendFilesToRegistry(setOfOriginalSRFiles);
				// if any legacy files were created as well, take the opportunity to delete them, and purge them from the database (better desigtn would to have not created them in the 1st place :( )
				if (!setOfLegacyOCRSRFiles.isEmpty()) {
					try {
				...
				try {
					List<Map<String,String>> returnedRecords = databaseInformationModel.findAllAttributeValuesForAllRecordsForThisInformationEntity(InformationEntity.STUDY);
					{
						for (Map<String,String> record : returnedRecords) {
							...
							boolean studyHasBeenProcessed = studyHasBeenProcessedValue != null && studyHasBeenProcessedValue.toUpperCase().equals("TRUE");
...
if (applicationDebugLevel > 1) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.ReceivedFileProcessor.run(): receivedFileName = "+receivedFileName);
				...
				AttributeList list = new AttributeList();
				list.read(i);	// don't stop at PixelData, since we may need to OCR it
				i.close();
				fis.close();
...
if (applicationDebugLevel > 1) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.ReceivedFileProcessor.run(): delete received file that was SR SOP Class that does not have X-Ray Radiation Dose Report document title");
						databaseInformationModel.insertObject(list,receivedFileName,DatabaseInformationModel.FILE_REFERENCED);			// record that we received it so that we don't keep retrieving it
...
if (applicationDebugLevel > 0) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.ReceivedFileProcessor.run(): isDoseScreenInstance");
						OCR ocr = new OCR(list,0/*debugLevel*/);
//System.err.print(ocr);
						ctDose = ocr.getCTDoseFromOCROfDoseScreen(ocr,ocrDebugLevel,null/*eventDataFromImages*/,true/*buildSR*/);
					}
...
if (applicationDebugLevel > 0) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.ReceivedFileProcessor.run(): isPhilipsDoseScreenInstance");
						ctDose = ExposureDoseSequence.getCTDoseFromPhilipsDoseScreen(list,ocrDebugLevel,null/*eventDataFromImages*/,true/*buildSR*/);
					}
					...
					if (ctDose != null) {
						...
						File ctDoseSRFile = storedFilePathStrategy.makeReliableStoredFilePathWithFoldersCreated(null,
                           savedImagesFolder,Attribute.getSingleStringValueOrDefault(ctDoseList,TagFromName.SOPInstanceUID,"1.1.1.1"));
...
if (applicationDebugLevel > 0) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.ReceivedFileProcessor.run(): adding our own newly created SR file = "+ctDoseSRFileName);
						ctDose.write(ctDoseSRFileName,ourCalledAETitle);	// has side effect of updating list returned by ctDose.getAttributeList(); uncool :(
						...
						databaseInformationModel.insertObject(list,receivedFileName,DatabaseInformationModel.FILE_REFERENCED);			// record that we received it so that we don't keep retrieving it
						// should we report that we received a file that either wasn't a candidate for OCR, or that we should have been able to OCR but couldn't ?
if (applicationDebugLevel > 1) System.err.println("DoseReporterWithLegacyOCRAndAutoSendToRegistry.ReceivedFileProcessor.run(): delete received file that we couldn't extract from "+receivedFileName);
						if (!new File(receivedFileName).delete()) {
							// OK to throw exception at this point, which will be trapped anyway, since no further processing to be done
							throw new DicomException("Failed to delete received file that we couldn't extract from "+receivedFileName);
						}
					}
				}
			}
